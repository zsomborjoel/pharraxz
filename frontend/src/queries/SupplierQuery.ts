...
export const useDeleteSupplier = (): UseMutationResult<void, unknown, string, unknown> =>
    useMutation(QUERIES.DELETE_SUPPLIER, (id: string) =>
        GenericService.del(ENDPOINTS.SUPPLIER, id).then((res) => res.data)
    );

export const useSaveProduct = (): UseMutationResult<Supplier, string, Supplier> =>
    useMutation(QUERIES.SAVE_SUPPLIER, (request: Supplier) =>
        GenericService.save(ENDPOINTS.SUPPLIER, request).then((res) => res.data)
    );
